//my additions:
var myturn = 'y';//can be y, n, or s(for spectator)
var id = 0;//an id for identifying self
var myMoveNum = 0;
var oppMoveNum = 0;
...
function getServerResponse() {
   ...
   xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
         ...
         var opponent = serverResponse.response[(id+1)%2];
         if(opponent.moveNum != oppMoveNum){
         	oppGameLoop(opponent.row, opponent.col);
         	oppMoveNum = opponent.moveNum;
				myturn = 'y';
				$('.turnMessage').stop().fadeIn(400).delay(2000).fadeOut(400);
         }
      }
   };
...
function sendData(r, c) {
   ...
   var data = JSON.stringify({"id":id, "moveNum":myMoveNum, "row":r, "col":c});
   xhttp.send(data);
}
